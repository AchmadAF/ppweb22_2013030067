...
if(isset($_POST['additem']) && $_POST['additem']=='Tambah'){
 ...
 $hpp = $_POST['hpp'];
	...
	$crt_date = date("Y-m-d H:i:s") ;
	    $harga = $_POST['harga'];
	$sql = "INSERT INTO item	(id,item_grp_id,nama,barcode,satuan_id,hpp,aktif,crt,crt_date,harga) VALUES('$id','$nk','$nama','$barcode','$satuan','$hpp','$aktif','$crt','$crt_date','$harga')";
...
if(isset($_POST['upditemgrp']) && $_POST['upditemgrp']=='Update'){
 ...
 $keterangan = $_POST['keterangan'];
	...
	$sql = "UPDATE item_grp
			set upline='$upline',nama='$nama',level='$level',keterangan='$keterangan',aktif='$aktif',upd='$upd',upd_date='$upd_date' WHERE id='$id'";
	if (!$res = $conn->query($sql))
		echo $sql;
	else
		header("location: item_grp.php");
